...
2. To calculate this test statistic, we need $SSE_{full}$ and $SSE_{reduced}$. We can get this with the `anova()` function in R. Run the following code:
...
3. Use the `anova` output to calculate the test statistic.

Under $H_0$, the test statistic follows an $F_{\# \ \text{parameters tested}, \ n-p}$ distribution.

4. Calculate $n - p$ and the number of parameters tested, for this data and these models.

Finally, we can calculate a p-value. To calculate a p-value for an F distribution, we use

```{r eval=F}
pf(..., df1 = ..., df2 = ..., lower.tail=F)
```

where the first argument is our test statistic (from exercise 3), and our second and third arguments arguments are the numerator and denominator degrees of freedom (from question 4).
